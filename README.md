...
- 原名优先：最终 YAML 默认保留上游节点名，只在名前添加订阅源 Emoji + 名称；重复名称追加 `#2`、`#3`。
...
更多安全边界和发版前安全检查见 [docs/security.md](docs/security.md)。
...
1. 提供静态二进制和 `init.d` 示例。
2. 验证配置目录、日志目录和 `/data` 持久化策略。
3. 再制作 `ipk` 包和 LuCI 集成。

当前 OpenWrt 相关说明见 [docs/03-openwrt-package.md](docs/03-openwrt-package.md) 和 [docs/10-luci-app.md](docs/10-luci-app.md)。
